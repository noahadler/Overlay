if Meteor.isClient
  @UploadProgress = new Mongo.Collection null
  ...
  FileRegistry.upload = @sendFile = (file, cb) ->
    ...
    sendSlice = (file, start, result) ->
      if start > file.size
        ...
        UploadProgress.remove {name: file.name}
        cb? result
        return
      slice = file.slice(start, start+sliceSize)
      console.log 'Sending slice '+start+' - '+(start+sliceSize)
      progress = UploadProgress.findOne {name: file.name}
      if progress
        UploadProgress.update progress._id, {$set: {uploaded: start} }
      else
